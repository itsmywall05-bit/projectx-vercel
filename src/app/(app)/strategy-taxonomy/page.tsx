...
const SK = "strat-catalog-v4";
async function loadCustoms(): Promise<Strategy[]> {
    if (typeof window === "undefined") return [];
    try {
        const raw = localStorage.getItem(SK);
        if (!raw) return [];
        const parsed = JSON.parse(raw) as Partial<Strategy>[];
        return parsed
            .map((c) => ({
                id: String(c.id ?? ""),
                sym: String(c.sym ?? ""),
...
async function saveCustoms(c: Strategy[]) {
    if (typeof window === "undefined") return;
    try {
